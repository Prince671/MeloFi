...
export default function Player({ id }) {
    ...
    const handleShare = () => {
        try {
            ...
            navigator.share({
                url: `${protocol}://${window.location.host}/${data.id}`
            });
        }
        catch (e) {
    ...
    useEffect(() => {
        // Fetch the current song details when the component mounts or `id` changes
        getSong();
        ...
        const handleSongEnd = async () => {
            if (next.nextData) {
                ...
                // Update the player state to play the next song
                await getSong(nextSongId); // Fetch the next song's data
                setPlaying(true); // Start playing the next song
            } else {
                setPlaying(false); // Stop if no next song is available
            }
        };
    ...
    useEffect(() => {
        const handleRedirect = () => {
            if (currentTime === duration && !isLooping && duration !== 0) {
                const protocol = window.location.protocol === "https:" ? "https" : "http";
                window.location.href = `${protocol}://${window.location.host}/${next?.nextData?.id}`;
            }
        };
